...
- `npm run db` - Start local PostgreSQL in Docker
...
- NEVER DO MIGRATION YOURSELF. Only run npm run generate when new types needed
...
6. Use npm for all package management
...
- CLI: Use `npm run dev:local-server` (NOT `npm run dev`) to load `.env.dev-local-server`
- Server: Use `npm run dev` to start with proper env files
